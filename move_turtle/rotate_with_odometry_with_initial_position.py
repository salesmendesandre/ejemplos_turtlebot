...
class RotateBot(Node):
    ...
    def odom_callback(self, msg):
        # Si la orientación objetivo aún no se ha establecido, establecerla como la orientación inicial más 90 grados
        if self.target_yaw_ is None:
            initial_yaw = math.atan2(2.0 * (msg.pose.pose.orientation.w * msg.pose.pose.orientation.z),
                                     1.0 - 2.0 * (msg.pose.pose.orientation.z * msg.pose.pose.orientation.z))
            self.target_yaw_ = initial_yaw + 1.5708  # 90 grados en radianes
